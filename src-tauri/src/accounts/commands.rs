...
use crate::accounts::queries::get_accounts;

#[tauri::command]
pub async fn get_all_accounts(state: tauri::State<'_, AppState>) -> Result<Vec<Account>, String> {
    let db = &state.db;

    let accounts = get_accounts(&db.0)
        .await
...
mod tests {
    ...
    async fn test_get_all_accounts(pool: Pool<Sqlite>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(result, get_expected_accounts());
        Ok(())
    }
}
